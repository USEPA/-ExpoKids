server <- function(input, output, session) {
  ...
  # no longer using - Steve Pfeiffer - Dec 2020.
  ...
  chemfile <- reactive({ # create reactive function of uploaded data
						req(input$inFile)
						inFile1 <- input$inFile
						datafile <- read_xlsx(inFile1$datapath, 1)
    
						chemfile2 <- data.frame(matrix(nrow = nrow(datafile), ncol = 3))
  ...
  output$p_perlife <- renderChart2({
    stack <- nPlot(Percent ~ Lifestage, group = "Media", data = mpct(),
                   type = "multiBarChart")
    stack$chart(stacked=T,color=colortab1,margin=list(left=85),reduceXTicks=F)
  ...
  observeEvent(input$button, {
							  mediaVal <- reactiveVal(input$media2)
							  if (length(mediaVal()) == 0)
								{
								output$p_addmedia <- renderText({ "Please select at least one media type..." })
								output$p_laddmedia <- renderText({ "Please select at least one media type..." })
								} else  {
										#######################################################################################################################
										####  ADD #############################################################################################################
										# get rid of previous UI objects - without these lines, the user might accidentally create overlapping graphs.
										removeUI("#padd1", immediate=TRUE)
										removeUI("#padd2", immediate=TRUE)
										removeUI("#padd3", immediate=TRUE)
										removeUI("#padd4", immediate=TRUE)
										removeUI("#padd5", immediate=TRUE)
										removeUI("#padd6", immediate=TRUE)
										removeUI("#padd7", immediate=TRUE)
										removeUI("#padd8", immediate=TRUE)
										removeUI("#padd9", immediate=TRUE)
										removeUI("#padd10", immediate=TRUE)
										# display UI object
										output$p_addmedia <- renderUI({
																	plotList <- lapply(1:length(mediaVal()), function(i) 	{
										...
										#shinyjs::runjs("alert(document.getElementById('padd1'));")
										#output$p_addmedia <- renderPrint({ sapply(plotList, class) })
										#
										# save plots into plotList
										meltchemfile <- meltchemfile()
										plotList <- list()
										for (i in 1:length(mediaVal())) {
																mediax <- subset(meltchemfile, meltchemfile$Media == mediaVal()[i]) #meltchemfile[my_i, 1])
																...
																plotm$set(title = paste0("", mediaVal()[i]))
										...
										#### LADD #############################################################################################################
										# get rid of previous UI objects - without these lines, the user might accidentally create overlapping graphs.
										removeUI("#pladd1", immediate=TRUE)
										removeUI("#pladd2", immediate=TRUE)
										removeUI("#pladd3", immediate=TRUE)
										removeUI("#pladd4", immediate=TRUE)
										removeUI("#pladd5", immediate=TRUE)
										removeUI("#pladd6", immediate=TRUE)
										removeUI("#pladd7", immediate=TRUE)
										removeUI("#pladd8", immediate=TRUE)
										removeUI("#pladd9", immediate=TRUE)
										removeUI("#pladd10", immediate=TRUE)
										# UI object
										output$p_laddmedia <- renderUI({
																	plotList1 <- lapply(1:length(mediaVal()), function(i) {
										...
										plotList1 <- list()
										for (i in 1:length(mediaVal())) {
																		  mediax <- subset(mwt, mwt$Media == mediaVal()[i])
																		  ...
																		  "http://timelyportfolio.github.io/rCharts_nvd3_templates/chartWithTitle_styled.html"
																		  plotl$set(title = paste0("", mediaVal()[i]))
